...
class Menu_saji extends MY_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('Mod_menu_saji'));
    ...
    public function index()
    {
        ...
        $data['menu'] = $this->Mod_menu_saji->getAll()->result();
        $this->template->load('layoutbackend','admin/menu_saji_data',$data);
    }

    public function ajax_list()
    {
        $list = $this->Mod_menu_saji->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $menusaji) {
            $no++;
            $row = array();
            $row[] = $menusaji->menu_gambar;
            $row[] = $menusaji->menu_nama;
            $row[] = $menusaji->menu_deskripsi;
            $row[] = $menusaji->menu_harga_lama;
            $row[] = $menusaji->menu_harga_baru;
            $row[] = $menusaji->menu_kategori_nama;
            $row[] = $menusaji->menu_id;
            $data[] = $row;
        }

        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Mod_menu_saji->count_all(),
            "recordsFiltered" => $this->Mod_menu_saji->count_filtered(),
            "data" => $data,
        );
    ...
    public function viewmenusaji()
    {
     $id = $this->input->post('id');
     $table = $this->input->post('table');
     $data_field = $this->db->field_data($table);
     $detail = $this->Mod_menu_saji->view_menusaji($id)->result_array();
     $data=array(
        'table'=>$table,
        'data_field'=>$this->db->field_data($table),
        'data_table'=> $detail,
